...
(function ($, Drupal, window, document) {
  ...
  Drupal.behaviors.dropsolidUILanguageMenu = {
      ...
      if (langNav.length) {
        ...
        if (typeof drupalSettings !== 'undefined' && typeof drupalSettings.theme_settings !== 'undefined' && typeof drupalSettings.theme_settings.language_dropdown !== 'undefined') {
          ...
          if (drupalSettings.theme_settings.language_dropdown === true ||  drupalSettings.theme_settings.language_dropdown === 1) {
            ...
            // TO DO: find way to use the theme settings variable in Twig
            //        so we don't have to do this stuff via JS
            //        + set a default value of TRUE via theme settings yml in config
            self.languageAbbr(langNav);
  ...
  /**
   * Replace text by abbreviations
   *
   */
  self.languageAbbr = function(nav) {
    ...
    // no need for the active language holder
    activeLangeHolder.hide();

    // replace text with abbreviation
    links.once('js-lang-link').each(function() {
      var myLink = $(this);
      myLink.text( myLink.attr('hreflang') );
    });
  };
