...
class PageManagerHelper {
   ...
   */
  public function getVariantTitle($page_manager_page_variant = NULL, Request $request = NULL) {
    $variant_title = '';
    if ($page_manager_page_variant != NULL) {
      ...
      $variant_title = $this->getTitle($page_manager_page_variant->getTitle(), $page_manager_page_variant->getContexts($request, TRUE));
    }
  ...
  protected function getContextAsTokenData(array $contexts) {
    ...
    foreach ($contexts as $context) {
      ...
      if (strpos($context->getContextDefinition()
          ->getDataType(), 'entity:') === 0
      ) {
        $token_type = substr($context->getContextDefinition()
          ->getDataType(), 7);
