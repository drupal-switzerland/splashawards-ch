...
namespace Drupal\dropsolid_event_core\Entity;
 ...
 *     "list_builder" = "Drupal\dropsolid_event_core\EventListBuilder",
 *     "views_data" = "Drupal\dropsolid_event_core\Entity\EventViewsData",
 *     "translation" = "Drupal\dropsolid_event_core\EventTranslationHandler",
 *
 *     "form" = {
 *       "default" = "Drupal\dropsolid_event_core\Form\EventForm",
 *       "add" = "Drupal\dropsolid_event_core\Form\EventForm",
 *       "edit" = "Drupal\dropsolid_event_core\Form\EventForm",
 *       "delete" = "Drupal\dropsolid_event_core\Form\EventDeleteForm",
 *     },
 *     "access" = "Drupal\dropsolid_event_core\EventAccessControlHandler",
 *     "route_provider" = {
 *       "html" = "Drupal\dropsolid_event_core\EventHtmlRouteProvider",
 *     },
 *   },
 *   base_table = "event",
...
class Event extends ContentEntityBase implements EventInterface {
  ...
  public function label() {
    $start = format_date($this->getStartDate(), 'custom', 'd/m/Y H:i');
    $end = format_date($this->getEndDate(), 'custom', 'd/m/Y H:i');
  ...
  public function buildDateComponent($date_format, $time_format, $class, $combine_duplicates = TRUE) {
    ...
    $datetime = format_date($start, 'custom', DATE_ATOM);
    if (date('d/m/Y', $start) === date('d/m/Y', $end) && $combine_duplicates) {
      $datetime .= '/' . format_date($end, 'custom', DATE_ATOM);
    }

    if ($date_format) {
      $start_date = format_date($start, 'custom', $date_format);
      $end_date = format_date($end, 'custom', $date_format);
    ...
    if ($time_format) {
      $start_time = format_date($start, 'custom', $time_format);
      $end_time = format_date($end, 'custom', $time_format);
