...
class BlockFieldItem extends FieldItemBase implements BlockFieldItemInterface {
  ...
  public function fieldSettingsForm(array $form, FormStateInterface $form_state) {
    ...
    $filter_options = [
      'plugin_ids' => $this->t('Blocks'),
      'categories' => $this->t('Categories'),
    ];

    $default_value_plugin_ids = $field->getSetting('plugin_ids') ?: array_keys($options);
    $default_value_categories = $field->getSetting('categories') ?: array_keys($category_options);
    $default_value_filter = $field->getSetting('filter') ?: reset($filter_options);
    ...
    $element['filter'] = [
      '#type' => 'radios',
      '#title' => $this->t('Filter'),
      '#description' => $this->t('Filter blocks by...'),
      '#options' => $filter_options,
      '#required' => TRUE,
      '#default_value' => $default_value_filter,
    ];
    ...
    $element['blocks'] = [
      ...
      '#open' => $field->getSetting('filter') == 'plugin_ids',
    ];
    $element['blocks']['plugin_ids'] = [
      ...
      '#default_value' => array_combine($default_value_plugin_ids, $default_value_plugin_ids),
    ];
    ...
    $element['categories'] = [
      ...
      '#open' => $field->getSetting('filter') == 'categories',
    ];
    $element['categories']['category_list'] = [
      ...
      '#default_value' => array_combine($default_value_categories, $default_value_categories),
    ];
    return $element;
  }
