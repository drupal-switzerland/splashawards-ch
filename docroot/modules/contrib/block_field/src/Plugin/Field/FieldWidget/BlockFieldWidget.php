...
class BlockFieldWidget extends WidgetBase implements ContainerFactoryPluginInterface {
  ...
  /**
   * Checks whether given plugin is allowed in field.
   *
   * @param string $plugin_id, array $definition
   *
   * @return bool
   */
  public function pluginIsAllowed($plugin_id, $definition) {
    $filter = $this->fieldDefinition->getSetting('filter');

    if ($filter == 'plugin_ids') {
      $plugin_ids = $this->fieldDefinition->getSetting('plugin_ids');
      return $plugin_ids && !isset($plugin_ids[$plugin_id]);
    }
    elseif ($filter == 'categories') {
      $allowed_categories = $this->fieldDefinition->getSetting('categories');
      $category = (string) $definition['category'];
      return $allowed_categories && isset($allowed_categories[$category]) && !$allowed_categories[$category];
    }
  }
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    foreach ($definitions as $id => $definition) {
      ...
      // included.
      if ($this->pluginIsAllowed($id, $definition)) {
    ...
    if (isset($definitions[$item->plugin_id]) && $this->pluginIsAllowed($item->plugin_id, $definitions[$item->plugin_id])) {
      $item->plugin_id = '';
      $item->setting = [];
    }
